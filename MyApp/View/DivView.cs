...
{
    public partial class DivView : Form
    {
        public DivView()
        {
            InitializeComponent();
        ...
        private void BtnDiv_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                double result = calc.Division(n1, n2);

                CalculatorRecordsHelper db = new();
                db.SaveRecord(n1, n2, result);
                FetchAllRecords();

                MessageBox.Show($"Division Success! Result: {result}", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox1.Clear();
                textBox2.Clear();
            }
            catch (Exception ex)
        ...
        {
            CalculatorRecordsHelper db = new();
            DataGridViewDiv.DataSource = db.GetAllRecords();
        }
    }
}
