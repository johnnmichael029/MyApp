...
{
    public partial class MulView : Form
    {
        public MulView()
        {
            InitializeComponent();
        ...
        private void BtnMul_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                double result = calc.Multiplication(n1, n2);

                CalculatorRecordsHelper db = new();
                db.SaveRecord(n1, n2, result);
                FetchAllRecords();

                MessageBox.Show($"Multiplication Success! Result: {result}", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox1.Clear();
                textBox2.Clear();
            }
            catch (Exception ex)
        ...
        {
            CalculatorRecordsHelper db = new();
            DataGridViewMul.DataSource = db.GetAllRecords();
        }
    }
}
